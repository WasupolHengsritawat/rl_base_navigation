...
class SimpleStageEnv(Node, gym.Env):
    def __init__(self, robot_id):
        rclpy.init()
        Node.__init__(self, 'stage_multi_robot_env')

        self.robot_id = robot_id
        self.agent_spawn_points = [[-5, 0], [0, 5], [5, 0], [0, -5]]
        ...
        self.cmd_vel_pubs = self.create_publisher(Twist, f'/robot_{self.robot_id}/cmd_vel', 10)
        self.lidar_subs = self.create_subscription(LaserScan, f'/robot_{self.robot_id}/base_scan', self.lidar_callback, 10) 
        self.odom_subs = self.create_subscription(Odometry, f'/robot_{self.robot_id}/odom', self.odom_callback, 10)
        self.rerandom_obj_vel_client = self.create_client(SetBool, f"/rerandom_req")
        # self.reset_state_client = self.create_client(Empty, f"/reset_positions")
    ...
    def start_stage(self):
        ...
        # Set robots goal
        self.goals = random_point_on_arc(self.agent_spawn_points[self.robot_id][0], self.agent_spawn_points[self.robot_id][1], GOAL_DISTANCE, GOAL_POOL_RADIUS)
        self.get_logger().info(f"robot_{self.robot_id} goals: ({self.goals[0]},{self.goals[1]})")

        self.robot_positions = np.array(self.agent_spawn_points[self.robot_id])
        self.robot_orientations = 0.0   # Not reliable since it is random in the simulation
        ...
        # Rerandom dynamic obstacles velocity
        rerand_req = SetBool.Request()
        rerand_req.data = True
        self.rerandom_obj_vel_client.call_async(rerand_req)

        # # Reset state
        # reset_req = Empty.Request()
        # self.reset_state_client.call_async(reset_req)
