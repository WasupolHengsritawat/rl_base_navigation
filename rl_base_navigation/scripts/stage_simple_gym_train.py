...
class SyncResetVecEnv(VecEnvWrapper):
    """Ensures all agents reset together and runs the reset service only once per reset."""

    def __init__(self, venv):
        super().__init__(venv)
        self.num_envs = venv.num_envs
        self.last_reset_called = False  # Flag to prevent multiple resets

    def reset(self):
        """Reset all environments together and call ROS reset service once."""
        if not self.last_reset_called:
            print("[INFO] Resetting positions via ROS service...")
            # subprocess.run(["ros2", "service", "call", "/reset_positions", "std_srvs/srv/Empty", "{}"], check=True)
            os.system("pkill stage_ros")  

            # Launch Stage
            subprocess.Popen(["ros2", "launch", "stage_ros2", "stage.launch.py", "world:=random_env"])

            self.last_reset_called = True  # Prevent redundant calls
    ...
    def step_wait(self):
        ...
        if np.any(dones):  # If any agent is done, reset all agents
            self.last_reset_called = False  # Allow reset on next call
            obs = self.reset()
...
if __name__ == '__main__':
    ...
    subprocess.Popen(["ros2", "launch", "stage_ros2", "stage.launch.py", "world:=simple_env"])
